...
static volatile uint32_t poti_adc_value_sum = 0;
static volatile uint32_t poti_adc_value_num = 0;
static int32_t poti_value = 0;
...
void poti_tick() {
	...
	if(system_timer_is_time_elapsed_ms(last_update, 1)) {
		...
		// We want "(poti_adc_value_sum*300 + 2*4095*poti_adc_value_num) / (4*4095*poti_adc_value_num)"
		// this is equivalent to the formula below, but we have less risk of overflow
		if(poti_adc_value_num > 0) {
			poti_value = 150 - ((poti_adc_value_sum*30 + 819*poti_adc_value_num) / (1638*poti_adc_value_num));
		}
...
int16_t poti_get_value(void) {
	return BETWEEN(-150, poti_value, 150);
}
